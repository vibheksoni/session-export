...
class CliApp:
    ...
    def run(self) -> int:
        ...
        if args.command == "list":
            store = {"codex": codex, "pi": pi, "opencode": opencode, "claude": claude, "devin": devin, "factory": factory}[args.provider]
            summaries = store.list(workers=args.workers or 1)
            self._print_summaries(summaries)
            return 0

        if args.command == "to-trace":
            return self._to_trace(args, codex, pi, opencode, claude, devin, factory)
        ...
        if args.command in ("codex-to-pi", "pi-to-codex", "codex-to-opencode",
                            ...
                            "devin-to-factory"):
            return self._single_convert(args, codex, pi, dcp, opencode, claude, devin, factory, id_factory)
    ...
    def _parser() -> argparse.ArgumentParser:
        ...
        list_parser.add_argument("provider", choices=("codex", "pi", "opencode", "claude", "devin", "factory"))
        ...
        export_all.add_argument("--targets", nargs="+", default=["pi"], choices=("pi", "opencode", "claude", "devin", "factory"))

        to_trace = subparsers.add_parser("to-trace")
        to_trace.add_argument("provider", choices=("codex", "pi", "opencode", "claude", "devin", "factory"))
    ...
    def _to_trace(
        ...
        stores = {"codex": codex, "pi": pi, "opencode": opencode, "claude": claude, "devin": devin, "factory": factory}
