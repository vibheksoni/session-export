...
|---|---|
| `provider` | Required. One of `codex`, `pi`, `opencode`, `claude`, `devin`. |
...
20 conversion commands, one for each ordered pair of tools:
...
```bash
python -m unisessions export-all --targets pi opencode claude devin [--write] [--workers N] [--on-conflict MODE]
```

| Flag | Description |
|---|---|
| `--targets {pi,opencode,claude,devin} [...]` | Target formats. Default: `pi`. |
...
| Argument | Description |
|---|---|
| `provider` | Source provider: `codex`, `pi`, `opencode`, `claude`, `devin`. |
